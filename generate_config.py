...
for loss_fn in ['cross_entropy','focal_loss']:
    for vpt_type in ['deep','shallow']:
        for opt in ['sgd','adam']:
            for lr in [0.001, 0.002, 0.005, 0.008, 0.01]:
                for wd in [0.0001, 0.0003, 0.0005,0.001,0.002]:
                    # if opt == 'adam':
                    #     continue
                    if vpt_type == 'shallow':
                        continue
                    ...
                    #     continue
                    with open(f'./configs/exps3/minghaocil_lr_{lr}_wd_{wd}_opt_{opt}_vt_{vpt_type}_loss_{loss_fn}.json', 'w') as f:
